...
# Agent LLM
from app.ai.agent import graph
import redis
...
class AskRequest(BaseModel):
    user_id: str = Field(default="default_user", description="Identifier to track history")
...
async def ask_agent(
    ...
    **Authentication:** Include header `X-API-Key: <your-key>`
    ...
    chat_history = []
    session_key = f"chat_history:{body.user_id}"
    
    if redis_client:
        history_str = redis_client.get(session_key)
        if history_str:
            chat_history = messages_from_dict(json.loads(history_str))
            
    chat_history.append(("human", body.question))
    ...
    try:
        ...
        final_answer = f"Sorry, there was an error processing your request: {str(e)}"
        
    if redis_client:
        # Save to Redis
        try:
            history_dict = messages_to_dict(chat_history)
            redis_client.setex(session_key, 86400, json.dumps(history_dict)) # Expire in 1 day
        except Exception as e:
